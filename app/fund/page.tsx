...
export default function FundPage() {
  ...
  const risk = RISK_OPTIONS.find(r => r.id === (config?.riskLevel ?? riskLevel)) ?? RISK_OPTIONS[1];
          ...
          <div className="rounded-xl p-4" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <div className="flex justify-between items-baseline mb-3">
              ...
              <span className="text-xs font-bold" style={{ color: "var(--primary)" }}>
                ${Math.round(projected5yr).toLocaleString()}
              </span>
            </div>
          ...
          {[
            ...
            <div key={s.label} className="rounded-xl p-4"
              ...
              <div className="text-xl font-bold mb-0.5"
                style={{ color: s.green ? "#1a6b4a" : "var(--foreground)" }}>{s.value}</div>
        ...
        <div className="px-5 mb-5">
          <div className="rounded-xl p-4" style={{ background: "var(--card)", border: "1px solid var(--border)" }}>
            <p className="text-xs font-semibold uppercase tracking-wider mb-4"
              style={{ color: "var(--muted-foreground)" }}>Milestones</p>
            {milestones.map(m => {
                ...
                <div key={m.label} className="mb-4 last:mb-0">
                  ...
                  <div className="rounded-full h-2" style={{ background: "var(--muted)" }}>
                    <div className="rounded-full h-2 transition-all"
                      style={{ width: `${pct}%`, background: reached ? "#1a6b4a" : "var(--primary)" }} />
                  </div>
                </div>
              );
            })}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-16">
        ...
        {step === 2 && (
          <div className="w-full max-w-md">
            ...
            <p className="text-sm mb-10" style={{ color: "var(--muted-foreground)" }}>
              We&apos;ll use this to calculate how much goes into your Housing Fund each month.
            </p>
        ...
        {step === 4 && (
          <div className="w-full max-w-md">
            ...
            <p className="text-sm mb-8" style={{ color: "var(--muted-foreground)" }}>
              Your Housing Fund will be invested on your behalf. Choose how much risk you&apos;re comfortable with.
            </p>
